...
static void myproto_server_init_int(myproto_server_t *userver, char *conn_str, pj_pool_t *pool) {
    userver->pool = pool;
...
#endif
    myproto_server_init_int(userver, conn_str, pool);
}
/*
...
static void open_udp_socket(myproto_server_t *userver, char *addr, int port) {
...
static void open_tty(myproto_server_t *userver, char *path) {
    (void)userver;
    (void)path;
#if 0
...
int myproto_unified_server_proc(void *param) {
    int ret;
    unsigned int len;
    ...
    myproto_server_t *userver = (myproto_server_t *)param;
            ...
            if( ret > 0 ) {
                ...
                if( pph != NULL ) {
                    ...
                    if( pj_ansi_strncmp(sts, plain, len1) == 0 ) {
                        ...
                        myproto_parse_request(plain, len1, &request);
                        userver->on_request_f(userver, &request, caddr_str);
                    }
                }
#else
                SHOW_LOG(5, "Received from client: %s\n", buffer);
                myproto_parse_request(buffer, ret, &request);
                userver->on_request_f(userver, &request, caddr_str);
#endif
            }
        }
...
int myproto_server_proc(void *param) {
    int ret;
    unsigned int len;
...
int myproto_secure_server_proc(void *param) {
    int ret;
    unsigned int len;
...
void myproto_server_start(myproto_server_t *userver) {
    pj_thread_create(userver->pool, NULL, myproto_unified_server_proc, userver, PJ_THREAD_DEFAULT_STACK_SIZE, 0, &userver->master_thread);
    //pj_thread_create(userver->pool, NULL, myproto_server_proc, userver, PJ_THREAD_DEFAULT_STACK_SIZE, 0, &userver->master_thread);
}

/*
void myproto_server_start_ex(myproto_server_t *userver) {
    pj_thread_create(userver->pool, NULL, myproto_secure_server_proc, userver, PJ_THREAD_DEFAULT_STACK_SIZE, 0, &userver->master_thread);
}
*/
...
void myproto_parse_show_table(json_object *jobj, char *buffer, int len, myproto_request_t *request) {
    json_object_object_foreach(jobj, key, val) {
    }
}
void myproto_parse_edit_table(json_object *jobj, char *buffer, int len, myproto_request_t *request) {
    json_object_object_foreach(jobj, key, val) {
        if(strcmp(key,"table_name") == 0) {
            strncpy(request->edit_table.table_name, json_object_get_string(val), sizeof(request->edit_table.table_name));
        }
    }
}
void myproto_parse_list_tables(json_object *jobj, char *buffer, int len, myproto_request_t *request) {
    json_object_object_foreach(jobj, key, val) {
        if(strcmp(key,"request") == 0) {
            strncpy(request->list_tables.request, json_object_get_string(val), sizeof(request->list_tables.request));
        }
    }
}
void myproto_parse_request(char *buffer, int len, myproto_request_t *request) {
    ...
    switch(msg_id) {
    case SHOW_TABLE:
        myproto_parse_show_table(jobj, buffer, len, request);
        break;
    case EDIT_TABLE:
        myproto_parse_edit_table(jobj, buffer, len, request);
        break;
    case LIST_TABLES:
        myproto_parse_list_tables(jobj, buffer, len, request);
        break;
    default:
        fprintf(stderr, "myproto_parse_request(): Invalid msg_id; %d\n", msg_id);
        exit(-1);
    }
}
